...
Or `pip install agent-worklog` in a regular Python environment.
...
Use `agent-worklog --help` for the command list. In scripts, name a subcommand
...
| [Releasing](https://github.com/mike840609/agent-worklog/blob/main/docs/releasing.md) | How a release is cut |
...
```bash
agent-worklog config init                                          # walk through all
agent-worklog config set harnesses.opencode.cli.model deepseek-r1  # write one
agent-worklog config list                                          # show all, with sources
agent-worklog config unset report.timezone                         # back to the default
```
